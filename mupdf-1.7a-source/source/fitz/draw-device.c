	...
	fz_colorspace *colorspace, float *color, float alpha)
{
	...
	dev->dumped_page->fz_begin_dump_fill_path(ctx, dev->dumped_page, path, even_odd, 
		ctm, colorspace, color, alpha);
	...
	dev->dumped_page->fz_end_dump_fill_path(ctx, dev->dumped_page);
}
	...
	fz_colorspace *colorspace, float *color, float alpha)
{
	...
	dev->dumped_page->fz_begin_dump_stroke_path(ctx, dev->dumped_page, path, stroke, ctm, 
		colorspace, color, alpha);
	...
	dev->dumped_page->fz_end_dump_stroke_path(ctx, dev->dumped_page);
}
	...
	fz_colorspace *colorspace, float *color, float alpha)
{
	...
	dev->dumped_page->fz_begin_dump_fill_text(ctx, dev->dumped_page, text, ctm, colorspace, color, alpha);
	...
	for (i = 0; i < text->len; i++)
	{
		...
		if (glyph)
		{
			...
			dev->dumped_page->fz_dump_char(ctx, dev->dumped_page, x, y, text->items[i].ucs, gid);
	...
	dev->dumped_page->fz_end_dump_fill_text(ctx, dev->dumped_page);
}
	...
	float *color, float alpha)
{
	...
	dev->dumped_page->fz_begin_dump_stroke_text(ctx, dev->dumped_page, text, stroke, ctm, 
		colorspace, color, alpha);
	...
	dev->dumped_page->fz_end_dump_stroke_text(ctx, dev->dumped_page);
}
...
fz_draw_fill_image(fz_context *ctx, fz_device *devp, fz_image *image, const fz_matrix *ctm, float alpha)
{
	...
	fz_try(ctx)
	{
		...
		dev->dumped_page->fz_begin_dump_fill_image(ctx, dev->dumped_page, pixmap, image, &local_ctm);

		fz_paint_image(state->dest, &state->scissor, state->shape, pixmap, &local_ctm, alpha * 255, !(devp->hints & FZ_DONT_INTERPOLATE_IMAGES));

		dev->dumped_page->fz_end_dump_fill_image(ctx, dev->dumped_page);
...
/**
fz_device *
fz_new_cj_device(fz_context *ctx, fz_pixmap *dest, fz_render_page *render_page)
{
	fz_draw_device *dev = (fz_draw_device *)fz_new_draw_device(ctx, dest);
	dev->render_page = render_page;
	return (fz_device*)dev;
}
*/
fz_device *
fz_new_cj_device(fz_context *ctx, fz_pixmap *dest, fz_dumped_page *dumped_page)
{
	fz_draw_device *dev = (fz_draw_device *)fz_new_draw_device(ctx, dest);
	dev->dumped_page = dumped_page;
	return (fz_device*)dev;
}
